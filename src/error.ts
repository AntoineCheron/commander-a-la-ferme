...
export class HttpError {
  constructor (public code: string | number, public description: string) {}
}
...
export function handleErrorsGlobally (
  f: () => Promise<void>,
  res: Response<any>
) {
  f().catch(error => res.status(toHttpStatus(error)).json(toHttpMessage(error)))
}
